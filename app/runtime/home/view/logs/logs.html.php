...
<div class="je-p20">
    ...
    <table class="je-table je-mb20" id="newCheck">
        ...
        <?php foreach( $log_list as $v ){ ?>
            <tr>
                ...
                <td><?php if( $v['action']==0 ){ ?>增加<?php }elseif( $v['action']==1 ){ ?>修改<?php }else{ ?>删除<?php } ?></td>
    ...
    <a <?php if( $pages-1==0 ){ ?>disabled<?php }else{ ?> href="logs?pages=<?=$pages-1?>&time=<?=$_GET['time']?>"<?php } ?> class="je-btn je-bg-green"><i class="je-icon je-f20">&#xe613;</i> 上一页</a>
    <button class="je-btn je-bg-red"><?php echo $pages;?></button>
    <a <?php if( count($log_list)!=20 ){ ?>disabled<?php }else{ ?> href="logs?pages=<?=$pages+1?>&time=<?=$_GET['time']?>"<?php } ?> class="je-btn je-bg-green">下一页 <i class="je-icon je-f20">&#xe60f;</i></a>
</div>
<script type="text/javascript">
    ...
    function search(){
        ...
        window.location.href = 'logs?time='+start+'|'+end;
    }
