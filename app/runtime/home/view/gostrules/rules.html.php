...
<div class="je-p20">
    ...
    <blockquote class="je-quote green je-f16 je-ovh  je-mb10">
        <p class="je-pb5 checkbox">
            ...
            <button class="je-btn" id="addart"><i class="je-icon je-f20">&#xe66e;</i> 添加gost转发</button>
    ...
    <table class="je-table je-mb20" id="newCheck">
    	...
    	<?php foreach( $rule_list as $v ){ ?>
        	<tr>
	            ...
	            <td><?php echo $v['listen_port'];?></td>
	            <td><?php echo $v['to_port'];?></td>
                <td><?php echo $v['type'];?></td>
                <?php $se = m('gost_server_list')->where(["id"=>$v['gost_server_id']])->find() ?>
                <td><?php echo $se["name"];?> -- <?php echo $se["server_key"];?></td>
...
<script type="text/javascript">
    ...
    function edit(id){
        ...
        url = '/gost/editrules?rule='+id+'&id=<?php echo $server_id;?>';
        jeBox.open({
            type: 'iframe',
            boxSize: ['70%', '85%'],
            maxBtn: true,
    ...
    jeui.use(["jquery","jeBox","jeDate","jeCheck","jeSelect"],function () {
        ...
        $("#addart").on("click",function(){
            jeBox.open({
                ...
                maxBtn: true,
                scrollbar: false,
                content: '/gost/addrules?id=<?php echo $server_id;?>'
            });
        })
        $(".myselect").jeSelect({
        	sosList: false,
        ...
        $("#switchself").jeCheck({
            ...
            itemfun:function(elem){
                ...
                var url = "/gost/rules?id=<?php echo $_GET['id']; ?>&self_add=" + is_self;
                window.location.href=url;
